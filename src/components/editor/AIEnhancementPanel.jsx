...
export default function AIEnhancementPanel({ project, onUpdate }) {
  ...
  const enhancements = [
    {
      ...
      icon: Maximize2,
      title: 'Auto Reframe',
      description: 'AI reframes to keep subjects centered',
      color: '#6366F1'
    },
    {
      id: 'noise_reduction',
    ...
    },
    {
      ...
      description: 'Cut out pauses and dead air',
      color: '#A855F7'
    },
    {
      id: 'text_suggestions',
      icon: Wand2,
      title: 'Text Overlay AI',
      description: 'Suggest optimal text placements & styles',
      color: '#EC4899'
    }
  ];
  ...
  const handleEnhance = async (enhancementId) => {
    ...
    try {
      const result = await base44.functions.invoke('enhanceVideo', {
        projectId: project.id,
        enhancementType: enhancementId,
        clips: project.edit_instructions?.clips || []
      });
      ...
      if (result.success) {
        ...
        toast.error(result.message || 'Enhancement failed');
      }
    } catch (error) {
    ...
    <div className="space-y-4">
        ...
        {enhancements.map((enhancement) => {
          ...
          return (
            <button
              ...
              className="w-full p-4 bg-[#18181B] border border-[#27272A] rounded-xl hover:border-[#3F3F46] transition text-left disabled:opacity-50"
            >
              <div className="flex items-start gap-3">
                <div 
                  className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                  style={{ backgroundColor: `${enhancement.color}20` }}
                >
                  {isProcessing ? (
                    <Loader2 className="w-5 h-5 animate-spin" style={{ color: enhancement.color }} />
                  ) : (
                    <Icon className="w-5 h-5" style={{ color: enhancement.color }} />
                  )}
                </div>
      ...
      <div className="p-3 bg-[#6366F1]/10 border border-[#6366F1]/20 rounded-lg">
        ...
        <div className="text-[#A1A1AA] text-xs">
          Apply enhancements before exporting for best results. Processing may take 1-2 minutes per enhancement.
        </div>
      </div>
    </div>
  );
