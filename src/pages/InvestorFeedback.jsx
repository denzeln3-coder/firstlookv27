...
export default function InvestorFeedback() {
  ...
  const submitFeedbackMutation = useMutation({
    mutationFn: async () => {
      await base44.entities.InvestorFeedback.create({
        pitch_id: pitchId,
        investor_id: user.id,
        rating,
        strengths,
      ...
      if (shareWithFounder && pitch.founder_id) {
        await base44.functions.invoke('createNotification', {
          userId: pitch.founder_id,
          type: 'system',
          message: isAnonymous 
            ? 'You received anonymous investor feedback on your pitch'
            : `An investor provided feedback on your pitch "${pitch.startup_name}"`,
          actionUrl: createPageUrl('ReceivedFeedback')
        });
      }
    },
    onSuccess: () => {
        ...
        <div className="max-w-3xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[#8E8E93] text-sm">{pitch?.startup_name}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="bg-[#0A0A0A] border border-[#18181B] rounded-xl p-6">
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map(value => (
              ...
              >
                <Star
                  className={`w-10 h-10 ${rating >= value ? 'text-[#F59E0B] fill-[#F59E0B]' : 'text-[#3F3F46]'}`}
                />
              </button>
            ))}
          </div>
        ...
        <div className="bg-[#0A0A0A] border border-[#18181B] rounded-xl p-6">
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              placeholder="e.g., Clear value proposition, strong team"
              className="flex-1 px-4 py-3 bg-[#18181B] text-white border border-[#27272A] rounded-xl focus:outline-none focus:border-[#6366F1]"
            />
            <button
              onClick={addStrength}
              className="px-4 py-3 bg-[#22C55E] text-white font-semibold rounded-xl hover:brightness-110 transition"
            >
        ...
        <div className="bg-[#0A0A0A] border border-[#18181B] rounded-xl p-6">
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              placeholder="e.g., Market size unclear, pricing needs work"
              className="flex-1 px-4 py-3 bg-[#18181B] text-white border border-[#27272A] rounded-xl focus:outline-none focus:border-[#6366F1]"
            />
            <button
              onClick={addImprovement}
              className="px-4 py-3 bg-[#F59E0B] text-white font-semibold rounded-xl hover:brightness-110 transition"
            >
              <Plus className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="bg-[#0A0A0A] border border-[#18181B] rounded-xl p-6">
          ...
          <textarea
            ...
            placeholder="Share more detailed thoughts, suggestions, or insights..."
            className="w-full px-4 py-3 bg-[#18181B] text-white border border-[#27272A] rounded-xl focus:outline-none focus:border-[#6366F1] resize-none"
            rows={6}
          />
        </div>
        ...
        <div className="bg-[#0A0A0A] border border-[#18181B] rounded-xl p-6 space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setShareWithFounder(e.target.checked)}
              className="w-5 h-5"
            />
            <div>
        ...
        <button
          ...
          className="w-full py-4 bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] text-white font-bold rounded-xl hover:brightness-110 transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
