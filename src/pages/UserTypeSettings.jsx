  ...
  const updateUserTypeMutation = useMutation({
    mutationFn: async (userType) => {
      ...
      if (user.user_type === 'investor' && userType !== 'investor' && investorProfile) {
        await supabase.from('investor_profiles').update({ is_active: false }).eq('id', investorProfile.id);
      }
      
      const { error } = await supabase.from('users').update({ user_type: userType }).eq('id', user.id);
  ...
  return (
    <div className="min-h-screen bg-[#000000] pb-20">
      ...
      <div className="px-4 py-6 max-w-md mx-auto">
        <p className="text-[#8E8E93] text-center mb-6">Your current type: <span className="text-white font-semibold capitalize">{user.user_type || 'Not set'}</span></p>
            ...
            return (
              <button key={type.id} onClick={() => setSelectedType(type.id)} className={`w-full p-4 rounded-xl border-2 transition text-left ${selectedType === type.id ? 'border-[#6366F1] bg-[#6366F1]/10' : 'border-[#27272A] bg-[#0A0A0A] hover:border-[#3F3F46]'}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${type.color} flex items-center justify-center flex-shrink-0`}><Icon className="w-6 h-6 text-white" /></div>
        ...
        <button onClick={handleSave} disabled={updateUserTypeMutation.isPending} className="w-full px-6 py-3 bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] text-white font-semibold rounded-xl hover:brightness-110 transition disabled:opacity-50">{updateUserTypeMutation.isPending ? 'Saving...' : 'Save Changes'}</button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#18181B] rounded-2xl p-6 max-w-md w-full border border-[#27272A]">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full bg-[#F59E0B]/20 flex items-center justify-center"><AlertTriangle className="w-6 h-6 text-[#F59E0B]" /></div>
              <h3 className="text-white text-xl font-bold">Archive Investor Profile?</h3>
            </div>
            <p className="text-[#A1A1AA] text-sm leading-relaxed mb-6">Switching away from Investor will archive your investor profile. You can reactivate it later by switching back to Investor type.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(false)} className="flex-1 px-4 py-3 bg-[#27272A] text-white font-semibold rounded-xl hover:bg-[#3F3F46] transition">Cancel</button>
              <button onClick={() => { setShowConfirm(false); updateUserTypeMutation.mutate(selectedType); }} className="flex-1 px-4 py-3 bg-[#6366F1] text-white font-semibold rounded-xl hover:brightness-110 transition">Continue</button>
            </div>
          </div>
        </div>
      )}
