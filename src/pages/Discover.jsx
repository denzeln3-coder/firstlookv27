...
export default function Discover() {
  ...
  const { data: pitches = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allPitches.filter(p => p.is_published === true || !p.review_status);
    }
  });
  ...
  return (
    <div className="fixed inset-0 bg-[#000000]">
      ...
      <div
        ...
        className="h-full overflow-y-scroll snap-y snap-mandatory pt-16 pb-20"
        style={{ 
      ...
      >
        {pitches.map((pitch, index) => (
          <div
            key={pitch.id}
            className="w-full h-full snap-start flex items-center justify-center relative cursor-pointer"
          ...
          >
            {pitch.video_url ? (
              <video
                ...
                onError={(e) => {
                  ...
                  if (parent && !parent.querySelector('.fallback-content')) {
                    ...
                    fallback.className = 'fallback-content w-full h-full flex items-center justify-center bg-gradient-to-br from-[#6366F1] to-[#8B5CF6]';
                    fallback.innerHTML = `<span class="text-white text-[64px] font-bold">${pitch.startup_name?.[0]?.toUpperCase()}</span>`;
              ...
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#6366F1] to-[#8B5CF6]">
                <span className="text-white text-[64px] font-bold">{pitch.startup_name?.[0]?.toUpperCase()}</span>
              </div>
            )}
            
            {/* Pitch Info Overlay */}
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent pointer-events-none">
              <h2 className="text-white text-[24px] font-bold mb-2">{pitch.startup_name}</h2>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[rgba(0,0,0,0.8)] backdrop-blur-xl border-t border-[rgba(255,255,255,0.06)] z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 8px)' }}>
        <div className="flex items-center justify-around py-2 px-4">
          <button 
            onClick={() => navigate(createPageUrl('Explore'))}
            className="flex flex-col items-center gap-1 text-[#636366] hover:text-[#FFFFFF] transition-colors duration-200 min-h-[44px] justify-center"
          >
            <Home className="w-6 h-6" />
            <span className="text-[10px] font-semibold tracking-[0.04em] uppercase">Home</span>
          </button>
          <button 
            onClick={() => navigate(createPageUrl('Discover'))}
            className="flex flex-col items-center gap-1 text-[#6366F1] min-h-[44px] justify-center"
          >
          ...
          >
            ...
            <span className="text-[10px] font-semibold tracking-[0.04em] uppercase">Top</span>
          </button>
          <button 
            onClick={async () => {
          ...
          >
            ...
            <span className="text-[10px] font-semibold tracking-[0.04em] uppercase">Record</span>
          </button>
          <button 
