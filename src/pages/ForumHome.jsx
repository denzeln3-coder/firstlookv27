...
import { ArrowLeft, Plus, Search, MessageSquare, Eye, Pin, Lock } from 'lucide-react';
...
export default function ForumHome() {
  ...
  const { data: threads = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const allThreads = await base44.entities.ForumThread.list('-last_post_at');
      if (selectedCategory === 'all') return allThreads;
      return allThreads.filter(t => t.category === selectedCategory);
    },
  ...
  const handleCreateThread = () => {
    if (!user) {
      base44.auth.redirectToLogin(createPageUrl('ForumHome'));
      return;
    }
      ...
      <div className="sticky top-0 bg-[#000000]/95 backdrop-blur-lg z-20 border-b border-[rgba(255,255,255,0.06)]">
        <div className="px-4 py-4 flex items-center justify-between">
          ...
          <button
            ...
            className="px-4 py-2 bg-gradient-to-br from-[#6366F1] to-[#8B5CF6] text-white text-sm font-semibold rounded-xl hover:brightness-110 transition flex items-center gap-2"
          >
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-[#1C1C1E] text-white text-sm border border-[rgba(255,255,255,0.06)] rounded-xl focus:outline-none focus:border-[#6366F1] placeholder:text-[#636366]"
            />
          </div>
        </div>
      ...
      <div className="px-4 py-6">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <button
              onClick={handleCreateThread}
              className="px-6 py-3 bg-gradient-to-br from-[#6366F1] to-[#8B5CF6] text-white font-semibold rounded-xl hover:brightness-110 transition"
            >
              Create Thread
            </button>
          </div>
          ...
          <div className="space-y-3">
            {filteredThreads.map((thread) => (
              <ForumThreadCard key={thread.id} thread={thread} users={allUsers} />
            ))}
          </div>
        )}
      </div>
          ...
          <div className="bg-[#18181B] border border-[rgba(255,255,255,0.1)] rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-[#0A0A0A] border border-[rgba(255,255,255,0.1)] rounded-xl text-white focus:outline-none focus:border-[#6366F1]"
                >
                  {categories.map((cat) => (
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={200}
                  className="w-full px-4 py-3 bg-[#0A0A0A] border border-[rgba(255,255,255,0.1)] rounded-xl text-white placeholder:text-[#636366] focus:outline-none focus:border-[#6366F1]"
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  className="w-full px-4 py-3 bg-[#0A0A0A] border border-[rgba(255,255,255,0.1)] rounded-xl text-white placeholder:text-[#636366] focus:outline-none focus:border-[#6366F1] resize-none"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 px-6 py-3 bg-[#27272A] text-white rounded-xl hover:bg-[#3F3F46] transition"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 px-6 py-3 bg-gradient-to-br from-[#6366F1] to-[#8B5CF6] text-white font-semibold rounded-xl hover:brightness-110 transition disabled:opacity-50"
              >
